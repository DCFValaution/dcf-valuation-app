...
void main() {
  group('successful export', () {
    test('returns bytes and the backend filename', () async {
      final api = apiReturning((_) => http.Response.bytes(
            _zipBytes,
            200,
            headers: {
              'content-type': _xlsx,
    ...
    test('falls back to a ticker-based filename when the header is absent',
        () async {
  ...
  group('refusals and errors', () {
    test('a not-suitable company is its own outcome, not a generic error',
        () async {
      final api = apiReturning((_) => http.Response(
            jsonEncode({
              'code': 'not_suitable',
    ...
    test('a plan limit is reported as such', () async {
      final api = apiReturning((_) => http.Response(
            jsonEncode({
              'code': 'plan_limited',
              'message': "BRK-B isn't available on your current data plan.",
            }),
            402,
          ));

      final result = await api.downloadExcel('BRK-B');
      expect((result as ExcelFailure).kind, ValuationFailureKind.planLimited);
      expect(result.message, contains('data plan'));
    });
    ...
    test('an unknown ticker says there is nothing to export', () async {
      ...
      expect((result as ExcelFailure).kind,
  ...
  group('content integrity', () {
    test('a JSON body arriving with a 200 is rejected, not saved as .xlsx',
        () async {
      ...
      expect((result as ExcelFailure).message, contains('not a valid .xlsx'));
    });
